// 各种面板：暂停（三页）、选卡、战利品、武器沙盒的控制面板、死亡结算（两页）、通关、回放角标。
//
// 和 hud.js 的分界线是"每帧都画 vs 一帧只画一个"：
// 局内 HUD 每帧都在画、要抠 canvas 调用数；面板是玩家停下来看的东西，一次只开一个，
// 关注点全在排版和信息密度上（这也是 UI 排版测试主要盯的地方）。
import { P } from '../shared/palette.js';
import { FONT } from './font.js';
import { VIEW_W, VIEW_H } from '../shared/viewport.js';
import { CARD_W, CARD_H, CARD_Y, cardX, banishBtn, REROLL_BTN, REPLAY_BTN, TAB_BTN, tabBtnX, EXIT_BTN,
  SANDBOX_PANEL, SANDBOX_HANDLE, SANDBOX_HANDLE_MIN, SANDBOX_ROW, sandboxRowRect,
  sandboxMinusRect, sandboxPlusRect, SANDBOX_BTN, sandboxBtnRect } from './layout.js';
import { MAX_SLOTS, findWeapon, WEAPONS } from '../content/weapons.js';
import { MAX_SKILL_SLOTS, findSkill, SKILLS } from '../content/skills.js';
import { findHero } from '../content/heroes.js';
import { DIFFICULTIES, findDifficulty, WIN_BONUS } from '../content/difficulty.js';
import { earnShards, ZONE_CLEAR_BONUS } from '../content/meta.js';
import { currentZone, ZONE_SECONDS, ZONES } from '../content/zones.js';
import { TRAITS, DASH } from '../core/sim.js';
...
// ui: { deps, best, meta, clock, WEAPON_NAME }
export function createPanels(ctx, ui) {
  const { deps, best, meta, clock, WEAPON_NAME } = ui;
  ...
  function drawPanelTabs(titles, active) {
    titles.forEach((name, i) => {
      ...
      ctx.font = on ? FONT.bold(13) : FONT.ui(13);
  ...
  function drawChoices(w) {
    ...
    w.choices.forEach((u, i) => {
      ...
      ctx.font = (u.evo || u.curse || u.skill) ? FONT.bold(20) : FONT.bold(26);
  ...
  function drawLoot(w) {
    ...
    w.loot.forEach((u, i) => {
      ...
      ctx.font = u.awaken ? FONT.bold(20) : FONT.bold(26);
      ctx.fillText(u.name, x + CARD_W / 2, CARD_Y + 62);
      ctx.fillStyle = P.dim;
      ctx.font = FONT.ui(14);
      const lines = u.desc.length > 14 ? [u.desc.slice(0, 13), u.desc.slice(13)] : [u.desc];
  ...
  return {
    drawPausePanel, drawChoices, drawLoot, drawSandbox, drawGameOver, drawWinPanel, drawReplayBadge,
    statBars,   // 结算面板的横条；渲染烟测会单独用它
  };
}
